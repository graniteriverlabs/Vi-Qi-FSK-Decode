...
static const NX_CRYPTO_METHOD* resolve_curve_method(USHORT group_id);
...
//        status = _nx_secure_x509_find_curve_method((USHORT)(ec_pubkey -> nx_secure_ec_named_curve), &curve_method);
        //Kannan
        curve_method = resolve_curve_method((USHORT)ec_pubkey -> nx_secure_ec_named_curve);
        if (curve_method == NX_CRYPTO_NULL)
        {
            return NX_SECURE_X509_UNSUPPORTED_PUBLIC_CIPHER; // same semantics as “not found”
        }
...
/* Resolve EC curve method using your linked tables. */
static const NX_CRYPTO_METHOD* resolve_curve_method(USHORT group_id)
{
    ...
    for (UINT i = 0; i < nx_crypto_ecc_supported_groups_size; ++i)
    {
        if (nx_crypto_ecc_supported_groups[i] == group_id)
        {
            return nx_crypto_ecc_curves[i];  // may still be NULL if not linked
        }
    }
