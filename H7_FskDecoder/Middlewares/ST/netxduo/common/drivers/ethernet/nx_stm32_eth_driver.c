...
/* Driver-level TX observability for debugger inspection. */
volatile ULONG g_eth_driver_tx_attempt_count = 0U;
volatile ULONG g_eth_driver_tx_submit_count = 0U;
volatile ULONG g_eth_driver_tx_submit_fail_count = 0U;
volatile ULONG g_eth_driver_tx_complete_count = 0U;
volatile ULONG g_eth_driver_tx_release_call_count = 0U;
volatile ULONG g_eth_driver_tx_release_buffers_in_use_before = 0U;
volatile ULONG g_eth_driver_tx_buffers_in_use = 0U;
volatile ULONG g_eth_driver_tx_buffers_high_water = 0U;
volatile ULONG g_eth_driver_last_deferred_events = 0U;
volatile ULONG g_eth_driver_last_error_code = 0U;
volatile ULONG g_eth_driver_last_dma_error_code = 0U;
volatile ULONG g_eth_driver_last_mac_error_code = 0U;
volatile ULONG g_eth_driver_last_dmacsr = 0U;
volatile ULONG g_eth_driver_last_macrxtxsr = 0U;
volatile ULONG g_eth_driver_last_mtltxqdr = 0U;
volatile ULONG g_eth_driver_hw_abnormal_count = 0U;
...
#endif /* NX_ENABLE_INTERFACE_CAPABILITY */
static VOID         _nx_driver_update_hw_status(VOID);
...
static VOID  _nx_driver_deferred_processing(NX_IP_DRIVER *driver_req_ptr)
{
  ...
  nx_driver_information.nx_driver_information_deferred_events =  0;
  g_eth_driver_last_deferred_events = deferred_events;
  ...
  if(deferred_events & NX_DRIVER_DEFERRED_PACKET_TRANSMITTED)
  {
    ...
    g_eth_driver_tx_release_call_count++;
    g_eth_driver_tx_release_buffers_in_use_before =
        HAL_ETH_GetTxBuffersNumber(&eth_handle);
    HAL_ETH_ReleaseTxPacket(&eth_handle);
    g_eth_driver_tx_buffers_in_use = HAL_ETH_GetTxBuffersNumber(&eth_handle);
    _nx_driver_update_hw_status();
  }
...
static VOID _nx_driver_update_hw_status(VOID)
{
  ULONG dmacsr = eth_handle.Instance->DMACSR;
  ULONG macrxtxsr = eth_handle.Instance->MACRXTXSR;
  ULONG mtltxqdr = eth_handle.Instance->MTLTQDR;

  g_eth_driver_last_dmacsr = dmacsr;
  g_eth_driver_last_macrxtxsr = macrxtxsr;
  g_eth_driver_last_mtltxqdr = mtltxqdr;

  if ((dmacsr & (ETH_DMACSR_AIS | ETH_DMACSR_FBE | ETH_DMACSR_CDE |
                 ETH_DMACSR_ETI | ETH_DMACSR_TBU | ETH_DMACSR_TPS)) != 0U)
  {
    g_eth_driver_hw_abnormal_count++;
  }

  if ((macrxtxsr & (ETH_MACRXTXSR_EXCOL | ETH_MACRXTXSR_LCOL |
                    ETH_MACRXTXSR_EXDEF | ETH_MACRXTXSR_LCARR |
                    ETH_MACRXTXSR_NCARR | ETH_MACRXTXSR_TJT)) != 0U)
  {
    g_eth_driver_hw_abnormal_count++;
  }
}
...
static UINT  _nx_driver_hardware_packet_send(NX_PACKET *packet_ptr)
{
  ...
  g_eth_driver_tx_attempt_count++;

  if(HAL_ETH_Transmit_IT(&eth_handle, &TxPacketCfg))
  {
    g_eth_driver_tx_submit_fail_count++;
    g_eth_driver_last_error_code = HAL_ETH_GetError(&eth_handle);
    g_eth_driver_last_dma_error_code = HAL_ETH_GetDMAError(&eth_handle);
    g_eth_driver_last_mac_error_code = HAL_ETH_GetMACError(&eth_handle);
    g_eth_driver_tx_buffers_in_use = HAL_ETH_GetTxBuffersNumber(&eth_handle);
    _nx_driver_update_hw_status();
    return(NX_DRIVER_ERROR);
  }

  g_eth_driver_tx_submit_count++;
  g_eth_driver_tx_buffers_in_use = HAL_ETH_GetTxBuffersNumber(&eth_handle);
  if (g_eth_driver_tx_buffers_in_use > g_eth_driver_tx_buffers_high_water)
  {
    g_eth_driver_tx_buffers_high_water = g_eth_driver_tx_buffers_in_use;
  }
  _nx_driver_update_hw_status();
...
void HAL_ETH_TxCpltCallback(ETH_HandleTypeDef *heth)
{
  ULONG deffered_events;
  deffered_events = nx_driver_information.nx_driver_information_deferred_events;
  g_eth_driver_tx_complete_count++;
  g_eth_driver_tx_buffers_in_use = HAL_ETH_GetTxBuffersNumber(heth);
  if (g_eth_driver_tx_buffers_in_use > g_eth_driver_tx_buffers_high_water)
  {
    g_eth_driver_tx_buffers_high_water = g_eth_driver_tx_buffers_in_use;
  }
